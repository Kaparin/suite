...
import { ArrowDownToLine, Coins, Gift, Check, Lock, ChevronLeft, ChevronRight, ArrowRight, ExternalLink } from 'lucide-react'
      ...
      <section className="py-16 md:py-20 glass-section">
        <div className="container-page max-w-5xl">
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid sm:grid-cols-3 gap-4 mb-10">
            {([
              { key: 'stake' as const, icon: <ArrowDownToLine className="w-6 h-6" /> },
              { key: 'earn' as const, icon: <Coins className="w-6 h-6" /> },
              { key: 'claim' as const, icon: <Gift className="w-6 h-6" /> },
            ]).map(({ key, icon }) => (
              <StatCard
                key={key}
                icon={icon}
