  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a12] to-[#12121e]">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 xl:ml-64">
        ...
        {/* ──── 4. STAKING ──── */}
        <motion.section id="staking" className="mb-20" {...fadeUp}>
          <h2 className="text-2xl font-bold text-white mb-2">{t('staking.title')}</h2>
          <p className="text-zinc-400 mb-8 max-w-2xl">
            {t('staking.subtitle')}
          </p>
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-6 mb-6">
            ...
            <div className="space-y-3">
              {[1, 2, 3].map(i => (
                <div key={i} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 text-xs font-bold flex items-center justify-center">{i}</span>
                  <div>
                    <p className="text-white font-medium text-sm">{t(`staking.step${i}Title`)}</p>
                    <p className="text-zinc-400 text-sm">{t(`staking.step${i}Desc`)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-6">
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
              <h4 className="text-white font-semibold mb-2">{t('staking.revenueTitle')}</h4>
              <p className="text-sm text-zinc-400">{t('staking.revenueDesc')}</p>
            </div>
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
              <h4 className="text-white font-semibold mb-2">{t('staking.modelTitle')}</h4>
              <p className="text-sm text-zinc-400">{t('staking.modelDesc')}</p>
            </div>
          </div>

          <Link href="/wallet?tab=staking" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 text-white font-medium rounded-xl transition-all">
            {t('staking.cta')}
