  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        className="bg-surface-1 border border-border rounded-[var(--radius-md)] p-6"
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-text-secondary mb-1">{t('panel.currentTier')}</p>
            <div className="flex items-center gap-3">
              {currentTier ? (
                <TierBadge tier={currentTier} size="md" />
              ) : (
                <span className="text-text-tertiary text-sm">{t('panel.noTier')}</span>
              )}
            </div>
          </div>
          <div className="w-12 h-12 bg-violet-500/15 rounded-[var(--radius-md)] flex items-center justify-center">
            <svg
      ...
      <motion.form
        ...
        className="bg-surface-1 border border-border rounded-[var(--radius-md)] p-6 space-y-5"
      >
        <h3 className="text-lg font-bold text-text-primary">{t('form.title')}</h3>
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-2.5 bg-surface-2 border border-border rounded-[var(--radius-sm)] text-text-primary focus:outline-none focus:ring-1 focus:ring-accent/30 focus:border-transparent transition-colors"
          >
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              min="1"
              step="any"
              className="w-full px-4 py-2.5 bg-surface-2 border border-border rounded-[var(--radius-sm)] text-text-primary placeholder-text-tertiary focus:outline-none focus:ring-1 focus:ring-accent/30 focus:border-transparent transition-colors pr-20"
            />
          ...
          <div className="grid grid-cols-4 gap-2">
            {DURATION_PRESETS.map((days) => (
              <button
                ...
                className={`px-3 py-2.5 rounded-[var(--radius-sm)] text-sm font-medium transition-all ${
                  durationDays === days
                    ? 'bg-accent text-text-primary shadow-sm'
                    : 'bg-surface-2 text-text-secondary border border-border hover:border-border hover:text-text-primary'
                }`}
              >
                {days}{t('form.days')}
              </button>
        ...
        {/* Live Tier Preview */}
        <div className="bg-surface-2 border border-border rounded-[var(--radius-sm)] p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm text-text-secondary">{t('form.tierPreview')}</span>
            {previewTier ? (
              <TierBadge tier={previewTier} size="md" />
            ) : (
              <span className="text-xs text-text-tertiary">{t('form.noTierPreview')}</span>
            )}
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {formError && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              className="bg-[var(--danger-bg)] border border-red-500/20 rounded-[var(--radius-sm)] px-4 py-3 text-sm text-[var(--danger)]"
            >
              {formError}
            </motion.div>
          )}
          {formSuccess && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              className="bg-emerald-500/10 border border-emerald-500/20 rounded-[var(--radius-sm)] px-4 py-3 text-sm text-emerald-400"
            >
              {formSuccess}
            </motion.div>
          )}
        ...
        <button
          ...
          className="w-full py-3 bg-accent hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed text-text-primary font-medium rounded-[var(--radius-md)] transition-all shadow-sm flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
      ...
      {locks.length > 0 && (
        <motion.div
          ...
          className="space-y-4"
        >
          <h3 className="text-lg font-bold text-text-primary">{t('locks.title')}</h3>
            ...
            {locks.map((lock) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-surface-1 border border-border rounded-[var(--radius-md)] p-5"
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {isExpiredOrViolated && (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium text-text-primary bg-surface-2 hover:bg-surface-3 border border-border rounded-[var(--radius-sm)] transition-colors disabled:opacity-50"
                      >
                        {unlockingId === lock.id ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    <div>
                      <p className="text-text-tertiary text-xs mb-0.5">{t('locks.amount')}</p>
                      <p className="text-text-primary font-medium">
                        {lock.amount.toLocaleString()} LAUNCH
                      </p>
                    </div>
                    <div>
                      <p className="text-text-tertiary text-xs mb-0.5">{t('locks.duration')}</p>
                      <p className="text-text-primary font-medium">
                        {lock.durationDays}{t('form.days')}
                      </p>
                    </div>
                    <div>
                      <p className="text-text-tertiary text-xs mb-0.5">{t('locks.endDate')}</p>
                      <p className="text-text-primary font-medium">{formatDate(lock.lockEndDate)}</p>
                    </div>
                    <div>
                      <p className="text-text-tertiary text-xs mb-0.5">
                        {isActive ? t('locks.timeLeft') : t('locks.wallet')}
                      </p>
                      <p className={`font-medium ${isActive ? 'text-accent' : 'text-text-secondary'}`}>
                        {isActive
