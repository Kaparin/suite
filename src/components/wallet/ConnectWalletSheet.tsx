        ...
        {showConnectionModal && (
          isMobile ? (
            ...
            >
              ...
              <div className="px-5 py-5 space-y-5">
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all text-base shadow-lg shadow-purple-500/20"
                >
                  {t('connectButton')}
                </button>
            ...
            <motion.div
              ...
              exit={{ x: '100%' }}
              transition={{ type: 'spring', damping: 30, stiffness: 300 }}
              className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-[420px] bg-gray-900 border-l border-gray-800 shadow-2xl flex flex-col"
            >
              {/* Header */}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                ...
                <div className="flex justify-center">
                  {qrValue ? (
                    ...
                    <div className="w-[252px] h-[252px] bg-gray-800 rounded-2xl flex items-center justify-center">
      ...
      {steps.map((step, i) => (
        <div key={step.num} className="flex gap-3">
          <div className="flex flex-col items-center">
            ...
            {i < steps.length - 1 && (
              <div className="w-px h-full min-h-[16px] bg-gray-700 mt-1" />
            )}
          </div>
          <div className="pb-2">
...
function AuthCodeBlock({
  ...
  if (!code) {
    return (
      <div className="p-3.5 bg-gray-800/80 rounded-xl border border-gray-700 text-center">
        <div className="flex items-center justify-center gap-2">
          <div className="w-4 h-4 border-2 border-purple-500/30 border-t-purple-500 rounded-full animate-spin" />
          <span className="text-sm text-gray-400">{generatingText}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      onClick={onCopy}
      className="p-3.5 bg-gray-800/80 rounded-xl border border-gray-700 cursor-pointer hover:border-purple-500/40 active:scale-[0.98] transition-all text-center"
    >
      <p className="text-base text-white font-mono tracking-wider select-all break-all">
        {code}
      </p>
...
function DownloadLink({ label }: { label: string }) {
  return (
    <a
      ...
      className="flex items-center justify-center gap-2 py-2 text-sm text-purple-400 hover:text-purple-300 transition-colors"
    >
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
